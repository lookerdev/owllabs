view: survey_data_nps_derived {
  label: "NPS - Salesforce"
  drill_fields: [id, nps_score_c]
  derived_table: {
    sql: SELECT * FROM salesforce.survey_data_nps
    WHERE
        {% condition device_filter %} survey_data_nps.self_selected_products_c {% endcondition %}
  ...
  filter: device_filter {
    # case_sensitive: no
    suggestions: ["Meeting Owl 3", "Whiteboard Owl"]
    type: string
    # sql: split_to_array( ${self_selected_products_c},',') ;;
  }
